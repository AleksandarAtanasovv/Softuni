...
const endpoints = {
  ...
  total: (offerId) => `/data/applications?where=offerId%3D%22${offerId}%22&distinct=_ownerId&count`,
  own: (offerId, userId) =>
...
describe("E2E tests", function () {
  ...
  describe("Navigation bar [ 5 Points ]", () => {
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async () => {
      ...
      expect(await page.isVisible("nav >> text=Dashboard"), "Dashboard is not visible").to.be.true;
      expect(await page.isVisible("nav >> text=Create Offer"), "Create is visible").to.be.false;
  ...
  describe("Home Page [ 10 Points ]", () => {
    ...
    it("Show home page [ 2.5 Points ]", async () => {
      ...
      expect(await page.isVisible("text=The right place for a new career start!")).to.be.true;
    });
  });
  ...
  describe("Dashboard Page [ 15 Points ]", () => {
    ...
    it("Check dashboard page with 2 offers [ 2.5 Points ]", async () => {
      ...
      const titles = await page.$$eval(".offer p .title", (t) => t.map((s) => s.textContent));
    ...
    it("Check dashboard page Info [ 2.5 Points ]", async () => {
      ...
      await page.waitForSelector("#dashboard");
      const titles = await page.$$eval(".offer p .title", (t) => t.map((s) => s.textContent));
      const salaries = await page.$$eval(".offer p .salary", (t) => t.map((s) => s.textContent));
  ...
  describe("CRUD [ 50 Points ]", () => {
    ...
    it("Check details information [ 2.5 Points ]", async () => {
      ...
      const title = await page.$$eval("#details-wrapper p", (t) => t.map((s) => s.textContent));
      const category = await page.$$eval("#details-wrapper p #categories", (t) =>
        t.map((s) => s.textContent)
      );
      const salary = await page.$$eval("#details-wrapper p #salary-number", (t) =>
        t.map((s) => s.textContent)
      );
  ...
  describe("BONUS : Apply functionality  [ 15 Points ]", async () => {
    ...
    it("Apply button should be hidden(not visible) after a click on it [ 2.5 Points ]", async () => {
      ...
      const [request] = await Promise.all([onRequest, page.click("#apply-btn")]);
    ...
    it("Apply button should increase total applications by 1 after a click on it [ 2.5 Points ]", async () => {
      ...
      let apps = await page.$$eval("#applications", (t) => t.map((s) => s.textContent));
      ...
      apps = await page.$$eval("#applications", (t) => t.map((s) => s.textContent));
