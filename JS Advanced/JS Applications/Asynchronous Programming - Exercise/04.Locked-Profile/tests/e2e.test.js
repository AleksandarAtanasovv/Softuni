...
describe("E2E tests", function () {
  ...
  describe("Profile Info", () => {
    it("Load profiles", async () => {
      ...
      const post = await page.$$eval(`.profile`, (t) => t.map((s) => s.textContent));
    ...
    it("Check profile name", async () => {
      ...
      const post = await page.$$eval(`input[name="user1Username"]`, (t) => t.map((s) => s.value));
    ...
    it("Check isLocked", async () => {
      ...
      const post = await page.$$eval(`input:checked`, (t) => t.map((s) => s.value));
    ...
    it("Check information when in unlock", async () => {
      ...
      await page.click('input[value="unlock"]');
      await page.click("text=Show more");
      const post = await page.$$eval(`input[type="email"]`, (t) => t.map((s) => s.value));
      expect(post[0]).to.equal(data[0].email);
      expect(post[1]).to.equal(`${data[1].age}`);
    });
  });
});
